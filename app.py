...
with st.sidebar:
    ...
    region = st.selectbox(
        "Select Study Region",
        ["Rajasthan", "Maharashtra", "Odisha", "Telangana"],
        index=0,
    )
    ...
    with col1:
        min_lat = st.number_input("Min Latitude", value=26.50, format="%.4f")
        min_lon = st.number_input("Min Longitude", value=73.50, format="%.4f")
    with col2:
        max_lat = st.number_input("Max Latitude", value=26.80, format="%.4f")
        max_lon = st.number_input("Max Longitude", value=73.80, format="%.4f")
